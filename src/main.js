...
if (tabs.length >= 3) {
    tabs[0].innerHTML = `⚔️ ${window.i18n('hud.log')}`;
    tabs[1].innerHTML = `💬 ${window.i18n('hud.chat')}`;
    tabs[2].innerHTML = `📖 ${window.i18n('hud.manual')}`;
}
...
document.querySelector('#tab-manual h3').innerText = `📖 ${window.i18n('hud.quick_guide')}`;
...
// Rotação alvo global das texturas para permitir histerese (evitar tremida visual VR)
let targetSnapAngle = 0;

// Função universal para resetar o tempo ocioso e devolver o telefone à aba original
...
// ==========================================/
// FUNÇÕES DA UI DO SMARTPHONE & CHAT LOG
// ==========================================/
...
function logBattle(pieceData, startX, startY, endX, endY, capturedPiece) {
    ...
    let logText = `<span style="color:${teamColor}; font-weight:bold;">${emoji} ${from} ➔ ${to}</span>`;
    if (capturedPiece) {
        const capEmoji = RANKS[capturedPiece.rank].icon;
        logText = `<span style="color:${teamColor}; font-weight:bold;">${emoji} ${from} ⚔️ ${capEmoji} ${to}</span>`;
    }
    const logDiv = document.getElementById('tab-log');
    const footer = document.getElementById('side-quest-footer');
    const entry = document.createElement('div');
    entry.className = 'log-entry';
    entry.innerHTML = logText;
    logDiv.insertBefore(entry, logDiv.firstChild);

    // Salta automaticamente para o Diário na primeira jogada
    if (!window.hasLoggedFirstMove) {
        ...
        if (activeTab && activeTab.id === 'tab-manual') {
            ...
            // Passa 'true' para registar que a partir de agora o Diário é o alvo principal do jogador
...
function showSideQuestFooter() {
    const logDiv = document.getElementById('tab-log');
    let footer = document.getElementById('side-quest-footer');
    if (footer) return; // Already visible
    footer = document.createElement('div');
    footer.id = 'side-quest-footer';
    footer.style.cssText = 'position: sticky; bottom: 0; background: linear-gradient(135deg, #2d1b0e, #4a2e15); border: 2px solid #FFD700; border-radius: 8px; padding: 8px 10px; margin: 8px 4px; color: #FFD700; font-size: 12px; font-family: monospace;';
    footer.innerHTML = `
                <div style="font-weight: bold; font-size: 13px; margin-bottom: 4px;">${window.i18n('hud.side_quest_title')}</div>
                <div style="color: #ccc;">${window.i18n('hud.side_quest_route_a')}</div>
                <div style="color: #ccc;">${window.i18n('hud.side_quest_route_b')}</div>
                <div style="color: #ccc;">${window.i18n('hud.side_quest_route_c')}</div>
            `;
...
import { RANKS, PARTICLE_EMOJIS, TEAMS, BOARD_COLS, BOARD_ROWS, WATER_TILES, TRAPS, DENS, INITIAL_SETUP, GameEngine, engine } from './engine.js';

import { initAudio, playSound } from './audio.js';

// ==========================================/
// THREE.JS SETUP & GRÁFICOS
// ==========================================/
...
// --- SISTEMA INTELIGENTE DE BOTÕES XR ---
...
function switchScene(newScene) {
    ...
    // Re-eval avaliar a visibilidade da mão caso a cena mude
...
// ==========================================/
// ALGORITMO DA INTELIGÊNCIA ARTIFICIAL (CPU)
// ==========================================/
...
function makeMonkeyMove() {
    if (currentScene !== 'game' || engine.currentTurn !== 2 || engine.winner !== null) {
        isCPUThinking = false;
        return;
    }

    // Replace the '2' checks with checks for monkey's current team since rewards adopt allies' teams
    const aliveMonkeys = engine.monkeyPieces ? engine.monkeyPieces.filter(m => !m.isDead && !m.isDying) : [];
    if (aliveMonkeys.length === 0) {
        engine.monkeyActive = false;
        engine.currentTurn = engine.nextTurnAfterMonkey !== undefined ? engine.nextTurnAfterMonkey : 0;
        isCPUThinking = false;
        return;
    }

    // Pick a random monkey to move this phase
    const monkey = aliveMonkeys[Math.floor(Math.random() * aliveMonkeys.length)];

    if (!engine.monkeyIsReward) {
        engine.monkeyRage--;
        monkey.rank = Math.max(0, engine.monkeyRage);
    }

    if (!engine.monkeyIsReward && engine.monkeyRage <= 0) {
        // Route A Resolution: Monkey calmed down
        let emptyTiles = [];
        for (let x = 0; x < BOARD_COLS; x++) {
            for (let y = 0; y < BOARD_ROWS; y++) {
                if (!engine.getPieceAt(x, y) && !engine.isWater(x, y) && !engine.isDen(x, y, 0) && !engine.isDen(x, y, 1)) {
                    emptyTiles.push({ x, y });
                }
            }
        }
        const bTile = emptyTiles.length > 0 ? emptyTiles[Math.floor(Math.random() * emptyTiles.length)] : { x: 3, y: 4 };

        const bPos = getWorldPos(bTile.x, bTile.y);
        bananaLeft.position.set(bPos.x, 0.5, bPos.z);
        bananaLeft.visible = true;

        selectedPiece = monkey;
        tryMove(bTile.x, bTile.y);

        monkey.routeAEndAnim = { phase: 0, timer: 0 };
        logChat(monkey, window.i18n('dialogues.monkey_calm_down') || "Minha banana! Obrigado... estou indo embora! 🍌❤️");
        window.localStorage.setItem('jungle_chess_macaco_reward', '2');

        isCPUThinking = false;
        return;
    }

    // Post chat message of current rage level (only if hostile)
    if (!engine.monkeyIsReward) {
        logChat(monkey, window.i18n('dialogues.monkey_rage_' + engine.monkeyRage) || `GRRR! (${engine.monkeyRage}) 🐒💢`);

        // Random piece replies to try and pacify the monkey
        const alivePieces = pieceMeshes.filter(p => !p.isDead && p.rank !== 9);
        if (alivePieces.length > 0) {
            const replier = alivePieces[Math.floor(Math.random() * alivePieces.length)];
            const pacifyNum = Math.floor(Math.random() * 3) + 1;
            setTimeout(() => {
                logChat(replier, window.i18n('dialogues.monkey_pacify_' + pacifyNum) || "Calm down!", monkey);
            }, 1500);
        }
    }
    ...
    for (let x = 0; x < BOARD_COLS; x++) {
        for (let y = 0; y < BOARD_ROWS; y++) {
            if (engine.isValidMove(monkey, monkey.gridX, monkey.gridY, x, y)) {
                let score = 0;
                const target = engine.getPieceAt(x, y);

                if (target && target.team !== monkey.team && target.team !== monkey.allyTeam) {
                    score += target.rank * 10;
                }

                // Try to move towards nearest enemy piece if no capture
                if (!target) {
                    let nearestDist = 999;
                    pieceMeshes.forEach(p => {
                        if (p.team !== monkey.team && p.team !== monkey.allyTeam && !p.isDead) {
                            const d = Math.abs(x - p.gridX) + Math.abs(y - p.gridY);
                ...
                possibleMoves.push({ piece: monkey, x, y, score });
            }
        }
    }
    ...
    if (possibleMoves.length > 0) {
        ...
        // Yield turn back to the correct player after move
        engine.currentTurn = engine.nextTurnAfterMonkey !== undefined ? engine.nextTurnAfterMonkey : 0;
    } else {
        engine.currentTurn = engine.nextTurnAfterMonkey !== undefined ? engine.nextTurnAfterMonkey : 0;
    }
...
// ==========================================/
// HUD ANCORADO NA CÂMARA E BALÕES DE FOFOCA (IM STYLE)
// ==========================================/
...
const menuHudBtn = createSquareHUDButton('☰', '#6B4226', '#000000');
...
// ==========================================/
// EASTER EGG PHASE 2: Banana Sprites
// ==========================================/
function createBananaSprite() {
    const canvas = document.createElement('canvas'); canvas.width = 256; canvas.height = 256;
    const ctx = canvas.getContext('2d');
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.font = '180px sans-serif';
    ctx.shadowColor = 'rgba(255, 220, 0, 0.8)'; ctx.shadowBlur = 30;
...
const bananaLeft = createBananaSprite();
bananaLeft.position.copy(posLeft); bananaLeft.position.y = 0.5;
bananaLeft.visible = false; gameGroup.add(bananaLeft);

const bananaRight = createBananaSprite();
bananaRight.position.copy(posRight); bananaRight.position.y = 0.5;
bananaRight.visible = false; gameGroup.add(bananaRight);

// Banana timer visuals in nest (per team)
let bananaTimerSprites = [null, null];
function createBananaTimerSprite(turnsLeft) {
    const canvas = document.createElement('canvas'); canvas.width = 256; canvas.height = 256;
    const ctx = canvas.getContext('2d');
    ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.font = '100px sans-serif'; ctx.fillText('🍌', 128, 90);
    ctx.font = 'bold 80px sans-serif'; ctx.fillStyle = '#fff'; ctx.strokeStyle = '#000'; ctx.lineWidth = 6;
    ctx.strokeText(turnsLeft.toString(), 128, 200); ctx.fillText(turnsLeft.toString(), 128, 200);
    const tex = new THREE.CanvasTexture(canvas);
    const sprite = new THREE.Sprite(new THREE.SpriteMaterial({ map: tex, transparent: true }));
...
function initPieces() {
    ...
    engine.reset();
    // Reset easter egg visuals
    secretTileLeft.visible = false; secretTileRight.visible = false;
    bananaLeft.visible = false; bananaRight.visible = false;
    bananaTimerSprites.forEach((s, i) => { if (s) { gameGroup.remove(s); bananaTimerSprites[i] = null; } });
    document.getElementById('side-quest-footer')?.remove();
    ...
    // Check Macaco-9 Rewards (from Routes A, B, C)
    let routeB = parseInt(window.localStorage.getItem('jungle_chess_macaco_reward_b') || "0", 10);
    let routeA = parseInt(window.localStorage.getItem('jungle_chess_macaco_reward') || "0", 10);
    let routeCStr = window.localStorage.getItem('jungle_chess_route_c_reward') || "";

    let routeC = null;
    let routeCCount = 0;
    if (routeCStr) {
        const parts = routeCStr.split('_');
        routeC = parseInt(parts[0], 10);
        routeCCount = parseInt(parts[1] || "0", 10);
    }

    let monkeySpawns = [];
    if (routeB > 0) {
        monkeySpawns.push({ x: 3, y: 1, ally: 0 });
        monkeySpawns.push({ x: 3, y: 7, ally: 1 });
        window.localStorage.setItem('jungle_chess_macaco_reward_b', (routeB - 1).toString());
        window.localStorage.removeItem('jungle_chess_macaco_reward');
    } else if (routeA > 0) {
        monkeySpawns.push({ x: 3, y: 1, ally: 0 });
        monkeySpawns.push({ x: 3, y: 7, ally: 1 });
        window.localStorage.setItem('jungle_chess_macaco_reward', (routeA - 1).toString());
    } else if (routeCCount > 0) {
        monkeySpawns.push({ x: 3, y: routeC === 0 ? 1 : 7, ally: routeC });
        window.localStorage.setItem('jungle_chess_route_c_reward', `${routeC}_${routeCCount - 1}`);
    }

    if (monkeySpawns.length > 0) {
        engine.monkeyActive = true;
        engine.monkeyIsReward = true;
        engine.monkeyPieces = [];
        monkeySpawns.forEach(setup => {
            const pos = getWorldPos(setup.x, setup.y);
            const group = new THREE.Group();
            group.position.set(pos.x, 0.4, pos.z);
            const mats = createPieceMaterial(9, setup.ally); // Visual Allied Team
            const cylinder = new THREE.Mesh(pieceGeo, mats);
            cylinder.castShadow = true; cylinder.receiveShadow = true; cylinder.rotation.y = setup.ally === 0 ? 0 : Math.PI;
            group.add(cylinder);
            const sprite = createEmojiSprite(9); sprite.visible = false; group.add(sprite);
            const hitBox = new THREE.Mesh(hitBoxGeo, hitBoxMat);
            ...
            const pieceData = {
                ...
                isMoving: false, moveProgress: 0, squashTimer: 0, capturedPiece: null, isVictoryPending: false,
                isDying: false, isDead: false, isMorphing: false, morphTimer: 0, isBigAnim: false, captureAnimTimer: 0,
                trapAnimState: 0, trapNumberVisual: 9, trapTimer: 0, isInDanger: false,
                baseTeamColor: new THREE.Color(TEAMS[setup.ally].color)
            };
            hitBox.userData = { isPiece: true, pieceData: pieceData };
            group.add(hitBox);
            gameGroup.add(group); pieceMeshes.push(pieceData); engine.addPiece(pieceData);
            engine.monkeyPieces.push(pieceData);
        });
    }

    // Clean up old formats
    if (window.localStorage.getItem('jungle_chess_macaco_reward') === 'true') window.localStorage.removeItem('jungle_chess_macaco_reward');
    if (window.localStorage.getItem('jungle_chess_macaco_reward_b') === 'true') window.localStorage.removeItem('jungle_chess_macaco_reward_b');
}
initPieces();
...
function showValidMoves(piece) {
    ...
    // Easter egg: secret tile highlights (banana tiles)
    if (engine.secretTileLeftActive && engine.isValidMove(piece, piece.gridX, piece.gridY, -1, 4)) {
        addHighlightAt(-1, 4, '#FFD700');
    }
    if (engine.secretTileRightActive && engine.isValidMove(piece, piece.gridX, piece.gridY, 7, 4)) {
        addHighlightAt(7, 4, '#FFD700');
    }
}
...
function handleInteraction(intersection) {
    ...
    if (obj.userData.isHUDButton) {
        ...
        if (obj.userData.action === '☰') { switchScene('menu'); }
        return;
    }
...
function tryMove(endX, endY, isRemote = false) {
    if (engine.isValidMove(selectedPiece, selectedPiece.gridX, selectedPiece.gridY, endX, endY)) {
        ...
        if (engine.secretTileLeftActive && !secretTileLeft.visible) {
            ...
            bananaLeft.visible = true;
            bananaLeft.position.y = 3.0; // Start high, will animate drop
            playSound('beep-up');
        }
        if (engine.secretTileRightActive && !secretTileRight.visible) {
            secretTileRight.visible = true;
            bananaRight.visible = true;
            bananaRight.position.y = 3.0; // Start high, will animate drop
            playSound('beep-up');
        }

        // Easter egg Phase 2: Banana collection — override animation
        if (moveResult.bananaCollected) {
            const returnPos = moveResult.bananaReturnPos;
            const returnWorld = getWorldPos(returnPos.x, returnPos.y);
            const isLeftSide = (endX === -1);
            const denPos = getWorldPos(3, selectedPiece.team === 0 ? 0 : 8);

            // Timer sprite will be created in Phase 5 when it actually activates
            showSideQuestFooter();

            selectedPiece.easterEggAnim = {
                phase: 0, // 0=move to tile, 1=banana flies, 2=tile breaks+fall, 3=fall to abyss, 4=fall from sky
                timer: 0,
                isLeftSide: isLeftSide,
                bananaMesh: isLeftSide ? bananaLeft : bananaRight,
                tileMesh: isLeftSide ? secretTileLeft : secretTileRight,
                denWorldPos: denPos,
                returnWorldPos: returnWorld,
                tileWorldPos: getWorldPos(endX, endY)
            };

            // Start normal move to tile
            ...
            selectedPiece.capturedPiece = null; selectedPiece.isVictoryPending = false;
        ...
        // Update banana timer visuals each turn (both teams)
        for (let t = 0; t < 2; t++) {
            const bt = engine.bananaTimers[t];
            if (bt.active && !bt.pending) {
                if (bananaTimerSprites[t]) gameGroup.remove(bananaTimerSprites[t]);
                const denPos = getWorldPos(3, t === 0 ? 0 : 8);
                bananaTimerSprites[t] = createBananaTimerSprite(bt.turnsLeft);
                bananaTimerSprites[t].position.set(denPos.x, 1.5, denPos.z);
                gameGroup.add(bananaTimerSprites[t]);
            } else if (!bt.active && bananaTimerSprites[t]) {
                gameGroup.remove(bananaTimerSprites[t]);
                bananaTimerSprites[t] = null;
            }
        }

        if (moveResult.routeATriggered && moveResult.routeABananaOrigin) {
            const origin = moveResult.routeABananaOrigin;
            const pos = getWorldPos(origin.x, origin.y);
            const group = new THREE.Group();
            group.position.set(pos.x, 0.4, pos.z);
            const mats = createPieceMaterial(9, 2);
            const cylinder = new THREE.Mesh(pieceGeo, mats);
            group.add(cylinder);
            ...
            const pieceData = {
                ...
                isMoving: false,
                moveProgress: 0,
                baseTeamColor: new THREE.Color(TEAMS[2].color)
            };
            hitBox.userData = { isPiece: true, pieceData: pieceData };
            group.add(hitBox);
            gameGroup.add(group);
            pieceMeshes.push(pieceData);
            engine.addPiece(pieceData);
            engine.monkeyPiece = pieceData;

            spawnCustomParticles(['💨', '✨', '🐒', '💢'], pos, 25);
            playSound('fall');
            setTimeout(() => logChat(pieceData, window.i18n('dialogues.monkey_angry_intro')), 1000);

            showSideQuestFooter();
            const routeAEl = document.getElementById('sq-route-a');
            if (routeAEl) {
                routeAEl.innerHTML = `<span style="color:#FF5252; font-weight:bold;">${window.i18n('hud.side_quest_route_a_res')}</span>`;
            }
        }

        if (moveResult.routeBTriggered) {
            // Visual trigger is deferred to easterEggAnim Phase 5
        }

        if (moveResult.routeCTriggered) {
            const winner = moveResult.winner;
            window.localStorage.setItem('jungle_chess_route_c_reward', `${winner}_2`);

            const ghostMonkey = { rank: 9, team: 2, baseTeamColor: new THREE.Color(TEAMS[2].color) };
            setTimeout(() => logChat(ghostMonkey, window.i18n('dialogues.monkey_route_c_promise') || "Estarei com o vencedor na próxima partida! 🐒⚔️"), 1500);

            showSideQuestFooter();
            const routeCEl = document.getElementById('sq-route-c');
            if (routeCEl) {
                routeCEl.innerHTML = `<span style="color:#FF9800; font-weight:bold;">${window.i18n('hud.side_quest_route_c_res')}</span>`;
            }
        }
...
function setupVRController(index) {
    ...
    // Fallback: Bastão/Cilindro elegante (melhor que o cubo antigo)
    const fallbackGroup = new THREE.Group();
    const handleGeo = new THREE.CylinderGeometry(0.015, 0.015, 0.12, 16);
    handleGeo.rotateX(Math.PI / 2); // Deitar o bastão para apontar para a frente
    ...
    // Escuta a conexão para ler o perfil do dispositivo de forma segura
    ...
    // Linha de laser visível saindo do comando
...
// --- INICIO DO MÓDULO DE PULSO (VR SMARTPHONE) ---
...
window.updateVRPhoneScreen = function () {
    ...
    vrPhoneCtx.fillText('⚔️ log', 85, 40);
    vrPhoneCtx.fillText('💬 net', 256, 40);
    vrPhoneCtx.fillText('📖 ebook', 427, 40);
    ...
    if (lastUserSelectedTabId === 'tab-manual') {
        ...
        vrPhoneCtx.fillText('📖 Guia Rápido', 30, 140 + vrPhoneScroll);
        vrPhoneCtx.font = '28px sans-serif';
        let y = 200 + vrPhoneScroll;
        const lines = [
            "O Alvo: Entre na Toca para vencer!",
            "Força: Elefante > Leão > Tigre ... Rato",
            "O Segredo: O Rato captura o Elefante!",
            "Água: Só o Rato pode entrar no rio.",
            "Salto: Leão e Tigre saltam sobre rios.",
            "Armadilha: Força cai a 0 na armadilha!"
        ];
        lines.forEach(l => {
            vrPhoneCtx.fillText('• ' + l, 30, y);
            y += 50;
        });
        ...
        logEntries.forEach(el => {
            ...
            vrPhoneCtx.fillText(el.innerText.replace('➔', '->'), 30, y);
            y += 45;
        });
...
window.setDominantHand = function (controller) {
    // Se for nulo, forçamos a releitura
...
function render(timestamp, frame) {
    ...
    if (currentScene === 'game' && !isCPUThinking && engine.winner === null && !restarting) {
        const isAnyPieceMoving = pieceMeshes.some(p => p.isMoving || p.isMorphing || p.squashTimer > 0 || p.trapAnimState > 0 || p.captureAnimTimer > 0 || p.isDying || (p.easterEggAnim && p.easterEggAnim.phase < 5));
        if (!isAnyPieceMoving) {
            if (engine.currentTurn === 0 && gameMode === 'cpu') {
                isCPUThinking = true;
                setTimeout(makeCPUMove, 1000);
            } else if (engine.currentTurn === 2) {
                isCPUThinking = true;
                setTimeout(makeMonkeyMove, 1000);
            }
        }
    }

    // =====================================
    // Dica 1: Fofocas Idle das Peças & Respostas (Retrucagens)
    // =====================================
    if (currentScene === 'game' && tipsEnabled && engine.winner === null && !restarting) {
        ...
        // Ajuste Dinâmico: Se o E-book estiver aberto, as peças "calam-se" por 60s
    ...
    // Easter egg: animate banana drop onto glass tiles
    if (bananaLeft.visible && bananaLeft.position.y > 0.55) {
        bananaLeft.position.y = THREE.MathUtils.lerp(bananaLeft.position.y, 0.5, delta * 4);
        bananaLeft.material.rotation = Math.sin(time * 8) * 0.3;
    }
    if (bananaRight.visible && bananaRight.position.y > 0.55) {
        bananaRight.position.y = THREE.MathUtils.lerp(bananaRight.position.y, 0.5, delta * 4);
        bananaRight.material.rotation = Math.sin(time * 8) * 0.3;
    }
    // Easter egg: banana bob animation when idle on tile
    if (bananaLeft.visible && bananaLeft.position.y <= 0.55) {
        bananaLeft.position.y = 0.5 + Math.sin(time * 3) * 0.08;
    }
    if (bananaRight.visible && bananaRight.position.y <= 0.55) {
        bananaRight.position.y = 0.5 + Math.sin(time * 3) * 0.08;
    }
    // Easter egg: banana timer bob (both teams)
    for (let t = 0; t < 2; t++) {
        if (bananaTimerSprites[t]) {
            bananaTimerSprites[t].position.y = 1.5 + Math.sin(time * 2 + t) * 0.1;
        }
    }
    ...
    pieceMeshes.forEach(pd => {
        ...
        // Easter Egg Phase 2: Multi-phase fall animation
        if (pd.easterEggAnim) {
            const anim = pd.easterEggAnim;
            anim.timer += delta;

            if (anim.phase === 0) {
                // Phase 0: Normal move to glass tile (handled by isMoving below)
                if (!pd.isMoving && pd.squashTimer <= 0) {
                    anim.phase = 1; anim.timer = 0;
                }
                // Don't return — let isMoving animation run below
            } else if (anim.phase === 1) {
                // Phase 1: Banana flies to the den
                const flySpeed = 4.0;
                const bPos = anim.bananaMesh.position;
                const dPos = anim.denWorldPos;
                bPos.x = THREE.MathUtils.lerp(bPos.x, dPos.x, delta * flySpeed);
                bPos.y = THREE.MathUtils.lerp(bPos.y, 1.5, delta * flySpeed);
                bPos.z = THREE.MathUtils.lerp(bPos.z, dPos.z, delta * flySpeed);
                anim.bananaMesh.material.rotation += delta * 15; // Spin!
                const dist = bPos.distanceTo(new THREE.Vector3(dPos.x, 1.5, dPos.z));
                if (dist < 0.15 || anim.timer > 2.0) {
                    // Banana rests at the den (stays visible)
                    bPos.set(dPos.x, 1.5, dPos.z);
                    anim.bananaMesh.material.rotation = 0;
                    anim.phase = 2; anim.timer = 0;
                    // Glass break!
                    playSound('glass_break');
                    spawnCustomParticles(['💎', '✨', '💫', '🔷', '❄️'], anim.tileWorldPos, 25);
                    anim.tileMesh.visible = false;
                    // Spawn ❓ above piece — EXACT same pattern as glass break particles above
                    const qCanvas = document.createElement('canvas'); qCanvas.width = 128; qCanvas.height = 128;
                    const qCtx = qCanvas.getContext('2d'); qCtx.font = '60px sans-serif'; qCtx.textAlign = 'center'; qCtx.textBaseline = 'middle'; qCtx.fillText('❓', 64, 64);
                    anim.questionSprite = new THREE.Sprite(new THREE.SpriteMaterial({ map: new THREE.CanvasTexture(qCanvas), transparent: true, opacity: 1 }));
                    anim.questionSprite.position.copy(anim.tileWorldPos);
                    anim.questionSprite.position.y = 1.5;
                    anim.questionSprite.scale.set(0.5, 0.5, 1);
                    gameGroup.add(anim.questionSprite);
                }
                return;
            } else if (anim.phase === 2) {
                // Phase 2: Coyote time — ❓ hovers, piece wobbles confused
                if (anim.questionSprite) {
                    // Bob up and down
                    anim.questionSprite.position.y = 1.5 + Math.sin(time * 6) * 0.15;
                    // Gentle scale pulse
                    const pulse = 1.0 + Math.sin(time * 4) * 0.15;
                    anim.questionSprite.scale.set(pulse, pulse, 1);
                }
                // Subtle confused wobble on the piece
                pd.mesh.rotation.z = Math.sin(anim.timer * 12) * 0.1;
                if (anim.timer > 2.0) {
                    // Remove ❓
                    if (anim.questionSprite) { gameGroup.remove(anim.questionSprite); anim.questionSprite = null; }
                    pd.mesh.rotation.z = 0;
                    anim.phase = 3; anim.timer = 0;
                    playSound('fall');
                }
                return;
            } else if (anim.phase === 3) {
                // Phase 3: Brief dramatic pause (looking down)
                if (anim.timer > 0.15) {
                    anim.phase = 4; anim.timer = 0;
                }
                return;
            } else if (anim.phase === 4) {
                // Phase 4: Piece falls into the abyss
                pd.mesh.position.y -= delta * 8;
                pd.mesh.rotation.x += delta * 4;
                pd.mesh.rotation.z += delta * 3;
                pd.mesh.scale.setScalar(Math.max(0.3, 1.0 - anim.timer * 0.3));
                if (pd.mesh.position.y < -10 || anim.timer > 2.8) {
                    anim.phase = 5; anim.timer = 0;
                    // Teleport above return position
                    const rw = anim.returnWorldPos;
                    pd.mesh.position.set(rw.x, 12, rw.z);
                    pd.mesh.rotation.set(0, 0, 0);
                    pd.mesh.scale.set(1, 1, 1);
                }
                return;
            } else if (anim.phase === 5) {
                // Phase 5: Piece falls from sky back to safety
                pd.mesh.position.y -= delta * 12;
                if (pd.mesh.position.y <= 0.4) {
                    pd.mesh.position.y = 0.4;
                    pd.targetPos.set(anim.returnWorldPos.x, 0.4, anim.returnWorldPos.z);
                    pd.startPos = pd.mesh.position.clone();
                    pd.squashTimer = 1.5;
                    pd.easterEggAnim = null;
                    playSound('land_safe');
                    spawnCustomParticles(['💨', '☁️', '✨'], anim.returnWorldPos, 15);

                    // Check if this collection resolved Route B
                    if (engine.routeBResolved) {
                        // Deferred Route B Truce trigger
                        const tpos = getWorldPos(3, 4);
                        const group = new THREE.Group();
                        group.position.set(tpos.x, 0.4, tpos.z);
                        const mats = createPieceMaterial(9, 2);
                        const cylinder = new THREE.Mesh(pieceGeo, mats);
                        group.add(cylinder);
                        ...
                        const hitBox = new THREE.Mesh(hitBoxGeo, hitBoxMat);
                        const pieceData = {
                            gridX: 3, gridY: 4, team: 2, rank: 9,
                            mesh: group, cylinder: cylinder, sprite: sprite, startPos: group.position.clone(), targetPos: group.position.clone(),
                            isMoving: false, moveProgress: 0, squashTimer: 0, capturedPiece: null, isVictoryPending: false,
                            isDead: false, isDying: false, isMorphing: false, morphTimer: 0, isBigAnim: false, captureAnimTimer: 0,
                            trapAnimState: 0, trapNumberVisual: 9, trapTimer: 0, isInDanger: false,
                            baseTeamColor: new THREE.Color(TEAMS[2].color)
                        };
                        hitBox.userData = { isPiece: true, pieceData: pieceData };
                        ...
                        if (bananaTimerSprites[0]) { gameGroup.remove(bananaTimerSprites[0]); bananaTimerSprites[0] = null; }
                        if (bananaTimerSprites[1]) { gameGroup.remove(bananaTimerSprites[1]); bananaTimerSprites[1] = null; }
                        if (bananaLeft.visible) bananaLeft.visible = false;
                        if (bananaRight.visible) bananaRight.visible = false;
                        if (secretTileLeft.visible) { spawnCustomParticles(['💨', '✨'], getWorldPos(-1, 4), 10); secretTileLeft.visible = false; }
                        if (secretTileRight.visible) { spawnCustomParticles(['💨', '✨'], getWorldPos(7, 4), 10); secretTileRight.visible = false; }

                        spawnCustomParticles(['✨', '🐒', '❤️', '🍌', '✌️'], tpos, 30);
                        playSound('win');
                        setTimeout(() => logChat(pieceData, window.i18n('dialogues.monkey_route_b_truce') || "Paz! Obrigado pelas bananas! ✌️🐒🍌"), 1000);

                        showSideQuestFooter();
                        const routeBEl = document.getElementById('sq-route-b');
                        if (routeBEl) {
                            routeBEl.innerHTML = `<span style="color:#25A02B; font-weight:bold;">${window.i18n('hud.side_quest_route_b_res')}</span>`;
                        }

                        window.localStorage.setItem('jungle_chess_macaco_reward_b', '2');

                        pieceData.routeBEndAnim = { phase: 0, timer: 0 };
                    } else {
                        // Normal banana grab: activate the banana timer for this team
                        const team = pd.team;
                        if (engine.bananaTimers[team].pending) {
                            engine.bananaTimers[team].active = true;
                            engine.bananaTimers[team].pending = false;
                            anim.bananaMesh.visible = false;
                            const denPos = getWorldPos(3, team === 0 ? 0 : 8);
                            if (bananaTimerSprites[team]) gameGroup.remove(bananaTimerSprites[team]);
                            bananaTimerSprites[team] = createBananaTimerSprite(engine.bananaTimers[team].turnsLeft);
                            bananaTimerSprites[team].position.set(denPos.x, 1.5, denPos.z);
                            gameGroup.add(bananaTimerSprites[team]);
                        }
                    }
                }
                return;
        ...
        if (pd.routeAEndAnim || pd.routeBEndAnim) {
            if (pd.isMoving || pd.squashTimer > 0) return; // Wait until arrived

            const anim = pd.routeAEndAnim || pd.routeBEndAnim;
            anim.timer += delta;

            if (anim.phase === 0) {
                if (anim.timer <= delta) { // just started
                    playSound('win');
                    spawnCustomParticles(['🍌', '✨', '🐒', '❤️'], pd.targetPos, 20);
                    if (bananaLeft.visible) bananaLeft.visible = false;
                    if (bananaRight.visible) bananaRight.visible = false;
                }

                pd.mesh.position.y += delta * 8;
                pd.mesh.rotation.y += delta * 15;
                pd.mesh.scale.setScalar(Math.max(0, 1.0 - anim.timer * 0.4));

                if (anim.timer > 2.5) {
                    pd.isDead = true;
                    pd.mesh.visible = false;
                    pd.routeAEndAnim = null;
                    engine.monkeyActive = false;
                    engine.currentTurn = engine.nextTurnAfterMonkey !== undefined ? engine.nextTurnAfterMonkey : 0;
        ...
        if (pd.trapAnimState === 1) {
            ...
            if (pd.trapTimer <= 0) {
                ...
                if (pd.trapNumberVisual >= 0) { updatePieceTopNumber(pd, pd.trapNumberVisual.toString()); playSound('beep'); pd.trapTimer = 0.15; } else {
                    ...
                    if (pd.isInDanger) { playSound('nervous'); spawnCustomParticles(['💧', '😰', '😱', '❗'], pd.targetPos, 15); }
                    else { playSound('cheeky'); spawnCustomParticles(['😈', '😼', '🔥', '🎵'], pd.targetPos, 15); }
                }
            }
        ...
        if (!pd.isDead && pd.cylinder && pd.cylinder.material) {
            ...
            // CORREÇÃO WEBXR: Emissividade mais suave nos óculos para evitar o efeito "Estouro" neon.
            if (isSelected) {
