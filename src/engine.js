...
export class GameEngine {
    ...
    hasAlliedMonkeys() { return this.pieces.some(p => p.rank === 9 && p.allyTeam !== undefined); }
    ...
    isValidMove(piece, startX, startY, endX, endY) {
        ...
        if (piece.rank === 9 && piece.allyTeam !== undefined && this.isDen(endX, endY, piece.allyTeam)) return false;

        const targetPiece = this.getPieceAt(endX, endY);
        if (targetPiece) {
            if (targetPiece.team === piece.team) return false;
            if (piece.rank === 9 && piece.allyTeam !== undefined && targetPiece.team === piece.allyTeam) return false;
        }
    ...
    executeMove(piece, endX, endY) {
        ...
        const canActivateEasterEgg = !this.easterEggConsumed && !this.hasAlliedMonkeys();
        // Easter egg activation: glass tile appears + banana
        if (endX === 0 && endY === 4 && !this.secretTileLeftActive && canActivateEasterEgg) {
            this.secretTileLeftActive = true;
            this.bananaLeftActive = true;
        }
        if (endX === 6 && endY === 4 && !this.secretTileRightActive && canActivateEasterEgg) {
        ...
        if (piece.team === 2) {
            if (piece.allyTeam !== undefined) {
                // Allied monkey
                const enemy = piece.allyTeam === 0 ? 1 : 0;
                if (this.isDen(endX, endY, enemy) || this.pieces.filter(p => p.team === enemy && p.rank !== 9).length === 0) {
                    winTriggered = true;
                    winningTeam = piece.allyTeam; // Passado para o aliado
                }
            } else {
                // Aggressive monkey
                if (this.isDen(endX, endY, 0) || this.isDen(endX, endY, 1)) {
                    winTriggered = true;
                    winningTeam = 2;
                }
            }
            ...
            // Normal player
            const enemy = piece.team === 0 ? 1 : 0;
            if (this.isDen(endX, endY, enemy) || this.pieces.filter(p => p.team === enemy && p.rank !== 9).length === 0) {
                winTriggered = true;
                winningTeam = piece.team;
            }
        }
        ...
        const enemyTeamForTurn = piece.team === 0 ? 1 : 0;

        if (this.monkeyActive) {
            // Save who's "really" next so monkey can yield the turn back
            if (piece.team !== 2) this.nextTurnAfterMonkey = enemyTeamForTurn;
            this.currentTurn = 2; // Monkey intercepts turn
        } else {
            this.currentTurn = enemyTeamForTurn;
        }
